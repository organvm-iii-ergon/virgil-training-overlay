...
func getSanitizedAppName(_ name: String?) -> String {
    ...
    // Using String(decoding:...) ensures valid UTF-8 sequences even if truncation cuts a multibyte character.
    let truncatedString = String(decoding: safeName.utf8.prefix(128), as: UTF8.self)

    // Optimization: Filter control characters by iterating unicode scalars.
    // This avoids the overhead of creating intermediate arrays and strings (components/joined).
    // Pre-allocating capacity avoids re-allocations during append.
    var result = ""
    result.reserveCapacity(truncatedString.utf8.count)
