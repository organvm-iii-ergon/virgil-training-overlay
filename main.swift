...
func getSanitizedAppName(_ name: String?) -> String {
    ...
    // Note: String(decoding:as:) is generally more robust for slicing utf8
    let truncated = String(decoding: safeName.utf8.prefix(128), as: UTF8.self)

    // Security: Remove control characters.
    // Optimization: Iterate unicodeScalars with reserveCapacity to avoid overhead of components(separatedBy:)
    var result = ""
